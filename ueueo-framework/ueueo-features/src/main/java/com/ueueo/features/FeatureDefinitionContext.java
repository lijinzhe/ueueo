...
public class FeatureDefinitionContext implements IFeatureDefinitionContext {
    ...
    public FeatureGroupDefinition addGroup(String name, ILocalizableString displayName) {
        ...
        if (groups.containsKey(name)) {
            throw new AbpException(String.format("There is already an existing feature group with name: %s", name));
        }
    ...
    public void removeGroup(String name) {
        ...
        if (groups.containsKey(name)) {
            ...
            throw new AbpException(String.format("Undefined feature group: '%s'.", name));
        }
    }
}
